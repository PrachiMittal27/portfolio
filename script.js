(() => {
  ...
  function initTvShowcase() {
    ...
    if (showcase && 'IntersectionObserver' in window) {
      ...
      // Separate, earlier observer purely to warm the file up: a
      // generous rootMargin fires it roughly a viewport before the set
      ...
      //
      // Deliberately built on `load`, not DOMContentLoaded: the set
      // sits about 1.5 viewports down, so a rootMargin wide enough to
      // be a useful buffering head start is also wide enough to
      // intersect at scroll position 0 — measured, it fires
      // immediately — which would put the fetch straight back in
      // contention with the hero. Waiting for `load` means the 32
      // tiles, Poppins, the three GSAP bundles and Lenis have all
      // been served first no matter what the viewport geometry works
      // out to, so the margin can stay generous without that
      // trade-off. Pixel/percentage tuning was the alternative and
      // it's fragile: the crossover depends on viewport height and on
      // how tall the WHY copy reflows at a given width.
      const startWarmup = () => {
  ...
  /* ---------------- Contact form -> Google Sheet ----------------
     Posts the form to the Apps Script web app named in the form's own
     data-endpoint (see the comment on it in Index.html, and
     contact-form-setup.md for the receiving script), then reports the
     result in place instead of navigating away.

     The body is sent as URLSearchParams rather than FormData on
     purpose: that makes the request Content-Type
     application/x-www-form-urlencoded, which is one of the three types
     the browser treats as a "simple" cross-origin request and so sends
     with no CORS preflight. A FormData body would trigger a preflight
     OPTIONS, which Apps Script doesn't answer — the submission would
     fail before it was ever sent, with nothing in the Sheet and a
     CORS error in the console.

     Reading the reply is treated as a bonus, never a requirement: the
     endpoint answers through a redirect to googleusercontent.com, and
     if that reply is ever unreadable the row has still been written by
     then. So anything short of an outright network failure is reported
     to the visitor as sent — the alternative is telling someone their
     message failed when it's actually sitting in the Sheet, which is
  ...
  function initContactForm() {
    ...
    // Not configured yet. Leaving the listener off entirely means the
    // form keeps its plain native behaviour rather than silently
    // swallowing a submission and claiming success — see step 7 of
    // contact-form-setup.md.
    if (!endpoint || endpoint === 'PASTE_WEB_APP_URL_HERE') {
      console.warn(
        '[contact form] No endpoint set — submissions are not being recorded. ' +
        'Paste the Apps Script web app URL into data-endpoint on the form in index.html ' +
        '(see contact-form-setup.md).'
      );
      return;
    }
    ...
    // button is dropped from the accessibility tree and loses focus
    // mid-interaction, which would silently move a keyboard user's
    // place on the page at the exact moment they pressed it.
    const setSending = (isSending) => {
      sending = isSending;
      if (submit) submit.setAttribute('aria-disabled', String(isSending));
    };
    ...
    form.addEventListener('submit', (event) => {
      ...
      // Let the browser's own validation (required, type="email") run
      // and show its native messages before anything is sent.
      ...
      fetch(endpoint, {
        ...
        // A readable JSON reply is the good case; an unreadable one
        // still means the request left and was accepted.
        .then((res) => res.json().catch(() => ({ ok: true })))
        .then((data) => {
          if (data && data.ok === false) throw new Error(data.error || 'rejected');
          form.reset();
          setStatus('Thank you, your message has been sent. I’ll be in touch soon.', false);
        })
        .catch(() => {
          setStatus(
